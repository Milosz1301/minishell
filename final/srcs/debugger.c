...
void    print_pipeline(t_pipe *pipeline)
{
	...
	while (pipeline)
	{
		...
		ft_printf("     ARGV:\n");
		ft_print_arr(pipeline->command->argv);
...
void    ft_print_envc(t_envar *envc)
{
	while (envc)
	{
		...
		ft_printf("		KEY: 		%s\n", envc->key);
		ft_printf("		VALUES: 	%s\n", envc->value);
		ft_printf("		EXP_STATUS:	%d\n", envc->is_exported);
		ft_printf("		NEXT:		%p\n", envc->next);
		ft_printf("--------------------------------------\n");
		envc = envc->next;
	}
}
void	ft_print_fd(pid_t pid, int fd, char *s, int c)
{
	if (pid == 0)
		ft_printf("IN CHILD ");
	if (c == 'o')
		ft_printf("Opened %s: %d\n", s, fd);
	else if (c == 'c')
		ft_printf("Closed %s: %d\n", s, fd);
	else if (c == 'd')
		ft_printf("Duplicated %s: %d\n", s, fd);
}

void	ft_print_envp(char	**envp)
{
	ft_printf("ENVP ARRAY---------------------\n");
	ft_print_arr(envp);
	ft_printf("-------------------------------\n");
}
